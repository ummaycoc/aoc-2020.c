...
static int isOccDir(int row, int col, int rAdj, int cAdj) {
  for(row += rAdj, col += cAdj; row >= 0 && row < numRows && col >= 0 && col < numCols; row += rAdj, col += cAdj)
    if (keep[row][col] == '#')
      return 1;
    else if (keep[row][col] == 'L')
      return 0;
  return 0;
}
...
static int countOcc(int row, int col) {
  int occ = 0;
  for (int rAdj = -1; rAdj <= 1; ++rAdj)
    for (int cAdj = -1; cAdj <= 1; ++cAdj)
      occ += (rAdj != 0 || cAdj != 0) && isOccDir(row, col, rAdj, cAdj);
  return occ;
}
  ...
  for (int r = 0; r < numRows; ++r)
    for (int c = 0; c < numCols; ++c)
      if (keep[r][c] == 'L' && countOcc(r, c) == 0) {
        update[r][c] = '#';
        ++count;
      } else if (keep[r][c] == '#' && countOcc(r, c) >= 5) {
        update[r][c] = 'L';
        ++count;
      }
  return count;
